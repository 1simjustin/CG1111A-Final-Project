...
float whiteArray[] = { 979.25, 986.5, 984.75 };
float blackArray[] = { 960.25, 902, 907.25 };
float greyDiff[] = { 19, 84.5, 77.5 };
...
void loop() {
  ...
  for (int c = 0; c <= 2; c++) {
    ...
    // digitalWrite(ledArray[c], 255);  //turn ON the LED, red, green or blue, one colour at a time.
    for (int zz = 0 ; zz < 2 ; zz++) {
    ...
    // digitalWrite(ledArray[c], 0);  //turn off the current LED colour
    for (int zz = 0 ; zz < 2 ; zz++) {
